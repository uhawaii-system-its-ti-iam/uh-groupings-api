...
public class GroupingsRestControllerv2_1 {
    ...
    /**
     * Determine whether user of uid is an owner.
     */
    @GetMapping("/owner/{uid:[\\w-:.]+}")
    @RequestMapping(value = "/owner/{uid:[\\w-:.]+}",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public ResponseEntity<GroupingsServiceResult> isOwner(@RequestHeader("current_user") String currentUser,
            @PathVariable String uid) {
        logger.info("Entered REST ownerGroupings...");
        return ResponseEntity
                .ok()
                .body(groupingAssignmentService.isOwner(currentUser, uid));
    }
