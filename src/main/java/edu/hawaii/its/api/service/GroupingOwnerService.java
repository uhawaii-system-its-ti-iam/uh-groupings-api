...
public class GroupingOwnerService {
    ...
    public GroupingMembers getGroupingMembersWhereListed(String currentUser, String groupingPath, List<String> uhIdentifiers) {
        ...
        for (int i = 0; i < hasMembersResultsBasis.getResults().size(); i++) {
            ...
            String whereListed = hasMemberResultBasis.getResultCode().equals("IS_MEMBER")
                    ? (hasMemberResultInclude.getResultCode().equals("IS_MEMBER") ? "Basis & Include" : "Basis")
                    : "Include";

            groupingMembers.add(new GroupingMember(hasMemberResultBasis.getSubject(), whereListed));
        }
    ...
    public GroupingMembers getGroupingMembersIsBasis(String currentUser, String groupingPath, List<String> uhIdentifiers) {
        ...
        for (HasMemberResult hasMemberResult : hasMembersResults.getResults()) {
            GroupingMember groupingMember = new GroupingMember(hasMemberResult.getSubject(),
                    hasMemberResult.getResultCode().equals("IS_MEMBER") ? "Basis" : "");
